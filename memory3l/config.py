...
# --------------------------------------------------------------------------- #
# This module is part of the installed package, so ``__file__`` no longer sits at
# the project root: the parent of the package directory is the checkout when
# running from source, and ``site-packages`` when installed from a wheel (where no
# .env exists and the lookup is harmlessly empty).  ``MEMORY3L_ENV_FILE`` pins the
# location explicitly for any other layout.
...
def _load_dotenv(path: Optional[str] = None) -> None:
    ...
    path = path or os.environ.get("MEMORY3L_ENV_FILE") or os.path.join(_PROJECT_ROOT, ".env")
